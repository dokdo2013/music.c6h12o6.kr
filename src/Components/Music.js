...

            {
              data.isLogin && (
                <Flex justifyContent="space-between" alignContent="center" mt="2">
                  <Button size="xs" colorScheme="gray" mr="0.5" onClick={() => { data.setEditTarget(apiData.idx); data.EditMusicModalOnOpen(); }} isFullWidth>수정</Button>
                  <Button size="xs" colorScheme="red" ml="0.5" onClick={() => { data.setDeleteTarget(apiData.idx); data.setDeleteIsOpen(true); }}><FaTrashAlt /></Button>
                </Flex>  
              )
            }
          </Box>
