    ...
    axios
      ...
      .then(Response => {
        if (Response.data.code === 'SUCCESS') {
          localStorage.setItem('X-Access-Token', Response.data.data['X-Access-Token']);
        ...
        <Stack align={'center'}>
          <Heading fontSize={'4xl'}>
            <Flex alignItems="center">
              ...
              <Badge fontSize="md" ml={1}>Admin</Badge>
            </Flex>
          </Heading>
        </Stack>
        <Box
              ...
              <Stack spacing={4}>
                ...
                <Stack spacing={10}>
                  <Stack
                    ...
                    <Checkbox ref={remember_id} size="sm" isChecked={isChecked} onChange={e => {
                      setIsChecked(e.target.checked)
                    }}>아이디 저장</Checkbox>
                    <Checkbox ref={auth_forever} size="sm"  onChange={e => {
                      if (e.target.checked) {
                        toast({
                          title: '\'로그인 상태 유지\' 설정은 공용 PC가 아닌 개인 PC에서만 사용해주세요',
                          status: 'warning',
                          isClosable: true,
                        })
                      }
